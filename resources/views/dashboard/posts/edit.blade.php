                    ...
                    <form action="{{route('posts.update', $post->id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input name="title" type="text" class="form-control @error('title') is-invalid @enderror" id="postTitle" placeholder="Post title" value="{{$post->title}}">
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <input name="slug" type="text" class="form-control @error('slug') is-invalid @enderror" id="postSlug" placeholder="Post slug" value="{{$post->slug}}">
                                @error('slug')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <textarea name="excerpt" class="form-control @error('excerpt') is-invalid @enderror" id="excerpt" placeholder="Post excerpt">{{$post->excerpt}}</textarea>
                                @error('excerpt')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input name="tags" type="text" class="form-control @error('tags') is-invalid @enderror" id="tags" placeholder="Tags" value="{{$post->tags}}">
                                @error('tags')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                <input class="form-control-checkbox" name="status" type="checkbox" id="status" @if($post->status == 1) checked @endif>
            ...
            function getSubcategories(){
                fetch('http://localhost/laravel-blog/dashboard/get-subcategories/'+categoryField.value)
                    ...
                    .then(subcategories => {
                        ...
                        let selectedId = {{$post->subcategory_id ? $post->subcategory_id: 0}};
                        subcategories.forEach(subcategory => {
                            let option = `<option value="${subcategory.id}" ${selectedId == subcategory.id ? 'selected':''}>${subcategory.subcategory_name}</option>`;
                            options += option;
                        });
